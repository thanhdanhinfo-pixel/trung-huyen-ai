...
class ObservabilityLayer:
    ...
    def capability_status(self):
        groups=system_awareness.capability_registry().get('capability_groups',{})
        return {k:v.get('status') for k,v in groups.items()}
    def memory_status(self):
        return {'self_state':'loaded','digital_twin':'loaded','memory_log':'available'}
    def github_status(self): return {'status':'connected'}
    def drive_status(self): return {'status':'active'}
