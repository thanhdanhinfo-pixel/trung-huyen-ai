...
class AdaptationEngine:
    def adapt(self):
        ...
        new_decision = {
            'id': f"D{len(decision_data['decisions']) + 1:03d}",
            'decision': 'apply_learned_patterns',
            'patterns': learned.get('rule_candidates', []),
            'date': str(date.today())
        }
        ...
        event_bus.publish('ADAPTATION_APPLIED', {
            'decision_id': new_decision['id'],
            'patterns': new_decision['patterns']
        })
