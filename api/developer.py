...
def developer_execute(req: DeveloperExecuteRequest):
    return workflow_engine.submit(action=f"developer.{req.action}",payload=req.payload,requires_approval=req.requires_approval,auto_run=req.auto_run)

@router.post("/developer/transform")
def developer_transform(req: TransformRequest):
    ops={"insert_import":code_transformer.insert_import,"insert_after":code_transformer.insert_after,"replace_block":code_transformer.replace_block,"replace_function":code_transformer.replace_function,"register_router":code_transformer.register_router}
